...
const int maxn = 1501, mod = 1000000007;
int n, ans;
void calc(char str[maxn + 1][maxn + 1])
{
	int top, sum;
	...
	for(int i = 1; i <= n; ++i)
	{
		...
		for(int j = 1; j <= n; ++j)
		{
			...
			no[top] = j;
			sum += hh[top] * (no[top] - no[top - 1]);
			ans = (ans + (LL)sum * (s[n - i][n - j] - s[n][n - j])) % mod;
		}
	}
}
int f[maxn + 1], stack[maxn][2], top, g[maxn][maxn];
