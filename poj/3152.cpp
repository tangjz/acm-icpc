...
int n, id, p, q, P[maxn];
void Maintain(map<int, Orders, greater<int> > &act, map<int, Orders, less<int> > &pass)
{
	while(q > 0)
	{
		if(pass.empty())
...
void Maintain(map<int, Orders, less<int> > &act, map<int, Orders, greater<int> > &pass)
{
	while(q > 0)
	{
		if(pass.empty())
			break;
		map<int, Orders, greater<int> >::iterator it = pass.begin();
		int pp = it -> first;
		greater<int> cmp;
		if(cmp(p, pp))
			break;
