...
inline LL mod_mul(LL x, LL y, LL m) { // (x * y) mod m
	return mod_sub(x * y, (LL)((DB)x * y / m + 0.01) * m, m);
}
...
pair<LL, LL> solve(LL n, LL m) {
	...
	all = mod_fact(n);
	now = mod_fact(m);
	now.first = mod_mul(now.first, all.first, pp);
	now.second += all.second;
	all = mod_fact(n + m);
	all.first = mod_mul(all.first, mod_inv(now.first, pp), pp);
...
int main()
{
	LL n, m;
	int sz;
	scanf("%lld%lld%d", &n, &m, &sz);
	ans = 0, mod = 1;
	init(2, sz);
	pair<LL, LL> all = solve(n, m);
	int lastp = p;
	LL lastpp = pp;
