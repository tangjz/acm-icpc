...
const int maxn = 9, maxm = 13, maxs = 423, maxp = 241, mod = (int)1e9 + 9;
struct HashTable {
...
inline bool inSet(int x, int y) {
	int dif = abs(x - y);
	return (dif <= 3 && mask & (1 << dif)) || (n - dif <= 3 && mask & (1 << (n - dif)));
}
int main() {
	scanf("%d", &n);
	for(int x; scanf("%d", &x) == 1; mask |= 1 << x);
	if(n <= 6) {
		int ans = 0, perm[7];
		for(int i = 0; i < n; ++i)
			perm[i] = i;
		do {
			bool chk = inSet(perm[0], perm[n - 1]);
			for(int i = 1; chk && i < n; ++i)
				chk &= inSet(perm[i], perm[i - 1]);
			ans += chk;
