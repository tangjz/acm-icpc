...
const int maxn = 1500001, maxm = 1500001, maxd = 21;
...
int main()
{
	...
	for(int i = 1; i <= m; ++i)
	{
		if(!e[i].size())
			continue;
		int pid = 0, ppsz = 0, psz = 0, cur = 0, pre = 1;
		f[cur][0][0] = 0;
		for(vector<pair<int, int> >::iterator it = e[i].begin(); it != e[i].end(); ++it)
		{
			...
			if(pid + 1 < cid)
			{
				for(int j = 0; j <= ppsz; ++j)
					for(int k = 0; k <= psz; ++k)
						upd(f[cur][0][0], f[cur][j][k] + min(j, k));
				ppsz = psz = 0;
			}
			cur ^= 1;
			pre ^= 1;
			for(int j = 0; j <= psz; ++j)
				memset(f[cur][j], 0x3f, (csz + 1) * sizeof(LL));
			for(int j = 0; j <= psz; ++j)
			{
				if(f[pre][0][j] == INF)
					continue;
				for(int k = 0; k <= j && k <= csz; ++k)
					upd(f[cur][j - k][csz - k], f[pre][0][j] + k);
			}
			for(int j = 1; j <= ppsz; ++j)
				for(int k = 0; k <= psz && k <= csz; ++k)
					upd(f[cur][0][csz - k], f[pre][j][k] + k);
			pid = cid;
			ppsz = psz;
			psz = csz;
		}
