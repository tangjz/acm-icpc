...
void tarjan(int u)
{
	...
	for(std::vector<int>::iterator it = e[u].begin(); it != e[u].end(); ++it)
	{
		...
		if(!tim[v])
		{
			...
			{ // iterate all return edges from the subtree of v
			...
			low_upd(u, best); // update for triconnect, that means there exists at least one w make u-w-low[0][w]-best, u-w-best, best-u possible, notice that low[0][x] may equal to low[1][x]
		}
