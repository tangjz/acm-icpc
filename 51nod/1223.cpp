...
int tot, prime[maxn], last[maxn], sqn, cnt, d[maxs], qL, qR;
LL n, T, tmp, ans;
...
void dfs(int dep, LL sum, int sgn)
{
	if(dep > cnt)
	{
		LL L = (qL - 1) / sum, R = qR / sum, TT = T / sum;
		for(LL x = max(TT / R, 1LL); x <= TT / L; ++x)
			tmp += sgn * x * (min(TT / x, R) - max(TT / (x + 1), L));
		return;
	}
	dfs(dep + 1, sum, sgn);
