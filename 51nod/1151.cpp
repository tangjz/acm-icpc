...
void CRT(LL r2, LL m2)
{
	LL &r = ans, &m = mod, r1 = ans, m1 = mod, x, y;
...
int main()
{
	while(scanf("%lld", &N) == 1)
	{
		...
		prod = f[0];
		fact_5 = mod_fact(N);
		fact_5.first = mod_mul(fact_5.first, mod_pow(mod_inv(2), fact_5.second));
		CRT(fact_5.first, pp);
		fact_2.second -= fact_5.second;
		p = 2, pp = 262144;
		fact_2.first = mod_mul(fact_2.first, mod_pow(mod_inv(5), fact_5.second));
		if(fact_2.second < n)
			for(int i = 0; i < fact_2.second; ++i)
				fact_2.first = mod_mul(fact_2.first, p);
		else
			fact_2.first = 0;
		CRT(fact_2.first, pp);
		printf("%lld\n", ans);
